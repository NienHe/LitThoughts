...
當初設計資料庫時要考慮安全性，用戶資料會儲存帳號和密碼，當然要避免儲存明碼，防止駭客或自己人盜取密碼。  
所以使用 Bcrypt 加密，本篇會說明 Bcrypt 加密好處。

### 為什麼使用 BCrypt？

 - 安全性高：BCrypt 採用了 Salt 和多次"Hash"的方式來保護密碼，有效防止常見的密碼破解攻擊，如彩虹表攻擊和暴力破解攻擊。
 - Salt：確保原始密碼經過 BCrypt 處理之後產生不同"Hash"值，即使用戶密碼相同，他們的"Hash"值也不相同。
 - 可調節的安全性：可以通過調整"Hash"複雜度來調節加密的安全性，使得密碼更難以被破解。
 - 易於使用：BCrypt 提供了簡單易用的 API，使得密碼的加密和驗證過程變得輕鬆。
